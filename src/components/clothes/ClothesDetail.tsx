...
import { type ClothesDto, ClothesTypeLabel } from '../../types/clothes';
...
export default function ClothesDetail({ clothes, onClickEdit }: Props) {
  ...
  return (
    ...
    >
      <CardMedia component="img" image={clothes.imageUrl} sx={{ height: '50%', objectFit: 'contain' }} />
      <CardContent sx={{ height: '40%' }}>
        ...
        >
          <Box key={clothes.type} sx={{ display: 'flex', flexDirection: 'row', gap: 1, alignItems: 'center', mb: 1 }}>
            ...
            <HorizontalScrollList>
              {Object.entries(ClothesTypeLabel).map(([key, value]) => (
                <Chip key={key} label={value} size="small" color={key === clothes.type ? 'primary' : 'default'} />
              ))}
            </HorizontalScrollList>
          </Box>
